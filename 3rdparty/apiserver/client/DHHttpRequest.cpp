...
DHHttpRequestInput::DHHttpRequestInput() {
    initialize();
}

DHHttpRequestInput::DHHttpRequestInput(QString v_url_str, QString v_http_method) {
    initialize();
    url_str = v_url_str;
    http_method = v_http_method;
}

void DHHttpRequestInput::initialize() {
    var_layout = NOT_SET;
    url_str = "";
    http_method = "GET";
}

void DHHttpRequestInput::add_var(QString key, QString value) {
    vars[key] = value;
}

void DHHttpRequestInput::add_file(QString variable_name, QString local_filename, QString request_filename, QString mime_type) {
    DHHttpFileElement file;
...
DHHttpRequestWorker::DHHttpRequestWorker(QObject *parent, QNetworkAccessManager *_manager)
...
DHHttpRequestWorker::~DHHttpRequestWorker() {
...
QMap<QString, QString> DHHttpRequestWorker::getResponseHeaders() const {
    return headers;
}

DHHttpFileElement DHHttpRequestWorker::getHttpFileElement(const QString &fieldname) {
    if (!files.isEmpty()) {
    ...
    return DHHttpFileElement();
}

QByteArray *DHHttpRequestWorker::getMultiPartField(const QString &fieldname) {
...
void DHHttpRequestWorker::setTimeOut(int timeOutMs) {
...
void DHHttpRequestWorker::setWorkingDirectory(const QString &path) {
    if (!path.isEmpty()) {
        workingDirectory = path;
    }
}

void DHHttpRequestWorker::setResponseCompressionEnabled(bool enable) {
    isResponseCompressionEnabled = enable;
}

void DHHttpRequestWorker::setRequestCompressionEnabled(bool enable) {
    isRequestCompressionEnabled = enable;
}

int  DHHttpRequestWorker::getHttpResponseCode() const{
    return httpResponseCode;
}

QString DHHttpRequestWorker::http_attribute_encode(QString attribute_name, QString input) {
...
void DHHttpRequestWorker::execute(DHHttpRequestInput *input) {
    ...
    if (input->var_layout == ADDRESS || input->var_layout == URL_ENCODED) {
        ...
        // add files
        for (QList<DHHttpFileElement>::iterator file_info = input->files.begin(); file_info != input->files.end(); file_info++) {
    ...
    if (input->request_body.size() > 0) {
        ...
        if(!isFormData && (input->var_layout != MULTIPART) && isRequestCompressionEnabled){
            request_content.append(compress(input->request_body, 7, DHCompressionType::Gzip));
        } else {
    ...
    if (DHHttpRequestWorker::sslDefaultConfiguration != nullptr) {
        request.setSslConfiguration(*DHHttpRequestWorker::sslDefaultConfiguration);
    }
...
void DHHttpRequestWorker::on_reply_finished(QNetworkReply *reply) {
    bool codeSts = false;
...
void DHHttpRequestWorker::on_reply_timeout(QNetworkReply *reply) {
...
void DHHttpRequestWorker::process_response(QNetworkReply *reply) {
...
QByteArray DHHttpRequestWorker::decompress(const QByteArray& data){
    
    Q_UNUSED(data);
    return QByteArray();
}

QByteArray DHHttpRequestWorker::compress(const QByteArray& input, int level, DHCompressionType compressType) {
...
QSslConfiguration *DHHttpRequestWorker::sslDefaultConfiguration;
